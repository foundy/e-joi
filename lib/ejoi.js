 ...
 * @property {boolean|string|Array} override Boolean은 전체 필드 대상, 문자열이면 해당 필드 대상, 배열이면 요소별 필드 대상
 */
 ...
 *
 * @param {Object|Object[]} schema 배열인 경우 요소 전체를 검사한다.
...
function eJoi(schema, options = {}, callback) {
  ...
  const opts = { nextRoute: true }; // extend된 옵션이 나올 경우로 일단 표기 Object.assign({}, defaults, options);
  ...
  return (req, res, next) => {
    ...
    console.log('schema', compiled.describe().children.body.meta);

    if (callback) { return callback(req, res, next, result); }

    result
      .then(value => options.nextRoute ? next('route') : next())
